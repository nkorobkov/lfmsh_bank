...
class FacAttendForm(AttendKernelForm):
  block = MyBlockField(
      label='Блок факультатива',
      queryset=AttendanceBlock.objects.filter(
          related_attendance_types__name__in=[
...
class SeminarKernelForm(AttendKernelForm):
  ...
  block = MyBlockField(
      ...
      queryset=AttendanceBlock.objects.filter(
          related_attendance_types__name__in=[
              AttendanceTypeEnum.seminar_pass.value
          ]),
      required=True,
      empty_label='Блок',
      to_field_name='name')
...
class LabKernelForm(forms.Form):
  value_1 = forms.IntegerField(
      label='Баксы первому пионеру', required=True, min_value=1)
  value_2 = forms.IntegerField(
      label='Баксы второму пионеру', required=True, min_value=1)
