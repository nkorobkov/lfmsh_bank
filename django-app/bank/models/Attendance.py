...
class Attendance(AtomicTransaction):
  receiver = models.ForeignKey(
      User,
      related_name='received_attendance',
  ...
  related_transaction = models.ForeignKey(
      Transaction, on_delete=CASCADE, related_name='related_attendance_atomics')

  @classmethod
  def new_attendance(cls,
                     receiver,
                     value,
                     type,
                     description,
                     date,
                     transaction,
                     attendance_block_name=None):
    attendance_block = AttendanceBlock.objects.get(
        name=attendance_block_name) if attendance_block_name else None
    new_att = cls(
  ...
  def is_valid(self):
    if not self.attendance_block:
      return True
    for suspicious in Attendance.objects.filter(receiver=self.receiver).filter(
        date=self.date).filter(counted=True).exclude(id=self.id).all():
