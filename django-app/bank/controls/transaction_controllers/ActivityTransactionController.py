...
class ActivityTransactionController(TableTransactionController):
  ...
  def get_initial_form_data(creator_username):
    initial = super(
        ActivityTransactionController,
        ActivityTransactionController).get_initial_form_data(creator_username)
        ...
        if reward:
          ...
          if form['cert'] and place == 1:
            Attendance.new_attendance(
                ...
                first_form['description'], form['date'], new_transaction)
  ...
  def _get_sport_reward(place):
    if place in [1, 2, 3, 4]:
      return ACTIVITY_REWARD[MoneyTypeEnum.sport_activity.value].get('single')[
          place - 1]
