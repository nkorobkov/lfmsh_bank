...
class AttendanceBlock(AbstractType):
  ...
  def clashes_with(self, other_block):
    if other_block:
      if (other_block.start_time <= self.start_time and self.start_time <
          other_block.end_time) or (other_block.start_time < self.end_time and
                                    self.end_time <= other_block.end_time):
        return True
    return False
