...
class FacAttendTransactionController(TableTransactionController):
  ...
  def get_initial_form_data(creator_username):
    initial = super(
        FacAttendTransactionController,
        FacAttendTransactionController).get_initial_form_data(creator_username)
  ...
  def get_transaction_from_form_data(formset_data, update_of):
    ...
    new_transaction = Transaction.new_transaction(
        creator,
        TransactionType.objects.get(name=TransactionTypeEnum.fac_attend.value),
