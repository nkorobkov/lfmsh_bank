...
urlpatterns = [
    ...
    url(r'^getTransactionHTML/$',
        views.get_transaction_HTML,
        name='get_transaction_HTML'),
    url(r'^students$', views.students, name='students'),
    url(r'^staff$', views.staff, name='staff'),
    url(r'^user/(?P<username>.+)/$', views.user, name='user'),
    url(r'^add_transaction/(?P<type_name>[^/]+)/tmp(?P<from_template>[0-9]+)/$',
        views.add_transaction,
        name='add_transaction_from_template'),
    url(r'^add_transaction/(?P<type_name>[^/]+)/upd(?P<update_of>[0-9]+)/$',
        views.add_transaction,
        name='update_transaction'),
    url(r'^add_transaction/(?P<type_name>.+)/$',
        views.add_transaction,
        name='add_transaction'),
    url(r'^decline/(?P<transaction_id>[0-9]+)/$', views.decline,
        name='decline'),
    url(r'^manage/(?P<user_group>[a-zA-z]+)/$', views.manage, name='manage'),
    url(r'^manage/(?P<user_group>[a-zA-z]+)/pr(?P<to_process>[0-9]+)$',
        views.manage,
        name='manage_process'),
    url(r'^manage/(?P<user_group>[a-zA-z]+)/dc(?P<to_decline>[0-9]+)$',
        views.manage,
        name='manage_decline'),
