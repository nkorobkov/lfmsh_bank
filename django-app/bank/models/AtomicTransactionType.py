...
class AtomicTransactionType(AbstractType):
  ...
  def full_info_as_list(self):
    return [self.readable_group_general, self.readable_group_local] + super(
        AtomicTransactionType, self).full_info_as_list()

  def full_info_headers_as_list(self):
    return ['type_group_general', 'type_group_local'] + super(
        AtomicTransactionType, self).full_info_headers_as_list()
