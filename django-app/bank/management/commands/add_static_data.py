...
class Command(BaseCommand):
  ...
  def add_transaction_states():
    ...
    for state in states:
      s, _ = TransactionState.objects.get_or_create(
          name=state['name'],
      ...
      for local_group_name, local_group_value in general_group_value.items():
        ...
        for type_name, type_readable_name in local_group_value.items():
          ...
          if model.objects.filter(name=type_name).exists():
            ...
            setattr(atomic_type, 'readable_group_local',
                    local_group_readable_name)
            setattr(atomic_type, 'readable_group_general',
                    general_group_readable_name)
            ...
            atomic_type = model.objects.create(
                ...
                name=type_name,
                readable_name=type_readable_name)
          atomic_type.save()
  ...
  def add_transaction_types(silent):
    ...
    for trans_type in types:
      new_type, _ = TransactionType.objects.get_or_create(
    ...
    for target in targets:
      for perm in per_group_permissions:
        ...
        for direction in direction_modificators:
          name = Command.make_perm_name(perm.value, target, direction,
                                        'transactions')
          content_type = ContentType.objects.get_for_model(TransactionType)
          new_perm = Permission.objects.get_or_create(
              codename=name, name=name, content_type=content_type)[0]
  ...
  def add_attendance_blocks(silent):
    ...
    for block_data in blocks_data:
      ...
      if AttendanceBlock.objects.filter(name=block_data['name']).exists():
        ...
        setattr(block, 'start_time',
                Command.time_from_string(block_data['start_time']))
        setattr(block, 'end_time',
                Command.time_from_string(block_data['end_time']))
