...
class ExamTransactionController(TableTransactionController):
  ...
  def get_transaction_from_form_data(formset_data, update_of):
    ...
    for atomic_data in formset_data:
      ...
      if value:
        ...
        Money.new_money(receiver, reward, money_type, first_form['description'],
                        new_transaction)
  ...
  def _get_exam_reward(value, scores):
    ...
    reward = max(0., (float(EXAM_BUDGET) * num_of_participants) *
                 normalized_score / (sum_normalized_score))
    return reward
